...
* **エンドポイント**: `GET /summary/weekly`
* **目的**: ホーム画面の棒グラフなど「直近7日間の家族全員の日次ポイント推移」を一括取得します。
* **クエリパラメータ**:
    * `endDate` (optional, `YYYY-MM-DD` 形式): 7日間ウィンドウの終端日（含む）。省略時は `Asia/Tokyo` 基準の今日。返却される `from` は `endDate` の6日前、`to` は `endDate` です。
* **レスポンス**:
    ```json
    {
      "from": "2026-05-17",
      "to": "2026-05-23",
      "summaries": [
        { "cognitoSub": "...", "date": "2026-05-17", "dailyPoints": 10 },
        { "cognitoSub": "...", "date": "2026-05-23", "dailyPoints": 30 }
      ]
    }
    ```
    * `summaries` は範囲内に存在する `DAILY` レコードのみ返します。家事実績が無い日付・ユーザーの組み合わせはレスポンスに含まれません（フロントエンド側で 0 にフォールバック）。
* **DynamoDB操作**: Query
    * PK: `FamilyID` を指定します。
    * SK: `BETWEEN 'DAILY#{from}' AND 'DAILY#{to}~'` を指定します（`~` (0x7E) は cognitoSub に出現する文字より常に大きいため、`DAILY#{to}#<任意のcognitoSub>` を含めつつ翌日のレコードは取り込みません）。
    * 実装のポイント: 範囲 Query なので1回のリクエストで7日 × メンバー人数分のレコードが取得できます。GSI/LSI は不要。

### 2.4.1. 月次サマリ取得API

* **エンドポイント**: `GET /summary/monthly`
* **目的**: カレンダー画面で、家族メンバーが「いつ家事をしたか」を月単位で俯瞰するためのデータを一括取得します。
* **クエリパラメータ**:
    * `month` (optional, `YYYY-MM` 形式): 取得したい月。省略時は `Asia/Tokyo` 基準の今月。
* **レスポンス**:
    ```json
    {
      "month": "2026-06",
      ...
      "to": "2026-06-30",
      "summaries": [
        { "cognitoSub": "...", "date": "2026-06-15", "dailyPoints": 30 }
      ]
    }
    ```
    * `summaries` は範囲内に存在する `DAILY` レコードのみ返します（家事実績が無い日は含まれない）。
    * `to` は閏年判定込みで月末日（28〜31）を返します。
* **エラー**: `400` `month` が `YYYY-MM` 形式でない
* **DynamoDB操作**: 週次サマリ (`§2.4`) と同じく `Query` SK `BETWEEN 'DAILY#{from}' AND 'DAILY#{to}~'`。範囲が広がるだけでアクセスパターンは同一。
...
### 2.6. 家事の作成・更新 API（家事マスターの Upsert）
...
* **パスパラメータ**: `taskId` (UUID)。`execute` という値はルーターが `DELETE /tasks/execute`（§2.11）に振り分けるため衝突しません。
* **リクエストボディ**: なし
