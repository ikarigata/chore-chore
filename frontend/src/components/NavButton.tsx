...
export default function NavButton({ icon: Icon, label, active, onClick }: NavButtonProps) {
  return (
    <button
      ...
      style={springStyle}
    >
      <div className={`relative p-1.5 rounded-lg ${active ? 'bg-brand-yellow border-2 border-stone-800' : 'text-stone-500'}`}>
        <Icon className={active ? 'w-5 h-5 text-stone-900' : 'w-6 h-6'} />
        {active && <div className="absolute -bottom-1 -right-1 w-2 h-2 bg-brand-teal rounded-full border border-stone-800"></div>}
      </div>
